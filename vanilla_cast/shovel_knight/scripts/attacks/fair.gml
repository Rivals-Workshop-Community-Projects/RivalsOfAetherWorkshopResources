set_attack_value(AT_FAIR, AG_CATEGORY, 1);

set_attack_value(AT_FAIR, AG_SPRITE, sprite_get("fair"));

set_attack_value(AT_FAIR, AG_HURTBOX_SPRITE, sprite_get("fair_hurt"));

set_attack_value(AT_FAIR, AG_NUM_WINDOWS, 7);
set_window_value(AT_FAIR, 1, AG_WINDOW_LENGTH, 4);
set_window_value(AT_FAIR, 1, AG_WINDOW_HSPEED, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_SFX, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_FAIR, 2, AG_WINDOW_HSPEED, 10);
set_window_value(AT_FAIR, 2, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_FAIR, 2, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FAIR, 2, AG_WINDOW_SFX, asset_get("sfx_dust_knuckle"));
set_window_value(AT_FAIR, 2, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_LENGTH, 3);
set_window_value(AT_FAIR, 3, AG_WINDOW_HSPEED, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_HSPEED_TYPE, 2);
set_window_value(AT_FAIR, 3, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_SFX, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_LENGTH, 8);
set_window_value(AT_FAIR, 4, AG_WINDOW_HSPEED, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_SFX, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_HAS_WHIFFLAG, 4);
set_window_value(AT_FAIR, 4, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_LENGTH, 4);
set_window_value(AT_FAIR, 5, AG_WINDOW_HSPEED, 10);
set_window_value(AT_FAIR, 5, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_HSPEED_TYPE, 1);
set_window_value(AT_FAIR, 5, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FAIR, 5, AG_WINDOW_SFX, asset_get("sfx_dust_knuckle"));
set_window_value(AT_FAIR, 5, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_FAIR, 6, AG_WINDOW_LENGTH, 3);
set_window_value(AT_FAIR, 6, AG_WINDOW_HSPEED, 0);
set_window_value(AT_FAIR, 6, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 6, AG_WINDOW_HSPEED_TYPE, 2);
set_window_value(AT_FAIR, 6, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 6, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_FAIR, 6, AG_WINDOW_SFX, 0);
set_window_value(AT_FAIR, 6, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 6, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_FAIR, 6, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_FAIR, 7, AG_WINDOW_LENGTH, 8);
set_window_value(AT_FAIR, 7, AG_WINDOW_HSPEED, 0);
set_window_value(AT_FAIR, 7, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 7, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_FAIR, 7, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 7, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_FAIR, 7, AG_WINDOW_SFX, 0);
set_window_value(AT_FAIR, 7, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 7, AG_WINDOW_HAS_WHIFFLAG, 4);
set_window_value(AT_FAIR, 7, AG_WINDOW_INVINCIBILITY, 0);
//ERROR_NO_SETTER_FOUND(AT_FAIR, HAS_COOLDOWN, 0);
//ERROR_NO_SETTER_FOUND(AT_FAIR, COOLDOWN, 0);
//ERROR_NO_SETTER_FOUND(AT_FAIR, HAS_WHIFF_LANDING, 0);
//ERROR_NO_SETTER_FOUND(AT_FAIR, AUTOCANCEL_FRAME, 0);
//ERROR_NO_SETTER_FOUND(AT_FAIR, LANDING_LAG, 0);
set_num_hitboxes(AT_FAIR, 2);
//ERROR_NO_SETTER_FOUND(AT_FAIR, NUM_UNIQUE_HITBOXES, 1);
//set_hitbox_value(AT_FAIR, 1, NUM, 0);
set_hitbox_value(AT_FAIR, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_FAIR, 2, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_FAIR, 1, HG_WINDOW, 3);
set_hitbox_value(AT_FAIR, 2, HG_WINDOW, 6);
set_hitbox_value(AT_FAIR, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FAIR, 2, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FAIR, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_FAIR, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_FAIR, 1, HG_BASE_KNOCKBACK, 5);
set_hitbox_value(AT_FAIR, 1, HG_KNOCKBACK_SCALING, 0.20);
set_hitbox_value(AT_FAIR, 1, HG_ANGLE, 361);
set_hitbox_value(AT_FAIR, 1, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FAIR, 1, HG_DAMAGE, 5);
set_hitbox_value(AT_FAIR, 1, HG_WIDTH, 92);
set_hitbox_value(AT_FAIR, 1, HG_HEIGHT, 44);
set_hitbox_value(AT_FAIR, 1, HG_HITBOX_X, 35);
set_hitbox_value(AT_FAIR, 1, HG_HITBOX_Y, -33);
set_hitbox_value(AT_FAIR, 2, HG_HITBOX_X, 35);
set_hitbox_value(AT_FAIR, 2, HG_HITBOX_Y, -33);
set_hitbox_value(AT_FAIR, 1, HG_EFFECT, 0);
set_hitbox_value(AT_FAIR, 1, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_FAIR, 1, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_FAIR, 1, HG_TECHABLE, 0);
set_hitbox_value(AT_FAIR, 1, HG_PRIORITY, 1);
set_hitbox_value(AT_FAIR, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_FAIR, 1, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_FAIR, 1, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_FAIR, 1, HG_VISUAL_EFFECT, 0);
set_hitbox_value(AT_FAIR, 1, HG_VISUAL_EFFECT_X_OFFSET, 18);
set_hitbox_value(AT_FAIR, 1, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_FAIR, 1, HG_HIT_SFX, asset_get("sfx_blow_weak2"));
set_hitbox_value(AT_FAIR, 1, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_FAIR, 1, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_FAIR, 1, HG_HIT_LOCKOUT, 3);
set_hitbox_value(AT_FAIR, 1, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_FAIR, 1, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_FAIR, 2, HG_HITBOX_GROUP, -1);
set_hitbox_value(AT_FAIR, 1, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_FAIR, 1, HG_SHAPE, 0);

set_attack_value(AT_DTILT, AG_CATEGORY, 3);

set_attack_value(AT_DTILT, AG_SPRITE, sprite_get("dtilt"));

set_attack_value(AT_DTILT, AG_HURTBOX_SPRITE, sprite_get("dtilt_hurt"));

set_attack_value(AT_DTILT, AG_NUM_WINDOWS, 5);
set_window_value(AT_DTILT, 1, AG_WINDOW_LENGTH, 9);
set_window_value(AT_DTILT, 1, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX, asset_get("sfx_swipe_weak1"));
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX_FRAME, 6);
set_window_value(AT_DTILT, 1, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_LENGTH, 2);
set_window_value(AT_DTILT, 2, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_SFX, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_LENGTH, 2);
set_window_value(AT_DTILT, 3, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTILT, 3, AG_WINDOW_SFX, asset_get("sfx_swipe_weak2"));
set_window_value(AT_DTILT, 3, AG_WINDOW_SFX_FRAME, 1);
set_window_value(AT_DTILT, 3, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DTILT, 4, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_SFX, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_LENGTH, 14);
set_window_value(AT_DTILT, 5, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_SFX, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_HAS_WHIFFLAG, 4);
set_window_value(AT_DTILT, 5, AG_WINDOW_INVINCIBILITY, 0);
//ERROR_NO_SETTER_FOUND(AT_DTILT, HAS_COOLDOWN, 0);
//ERROR_NO_SETTER_FOUND(AT_DTILT, COOLDOWN, 0);
set_num_hitboxes(AT_DTILT, 4);
//ERROR_NO_SETTER_FOUND(AT_DTILT, NUM_UNIQUE_HITBOXES, 4);
//set_hitbox_value(AT_DTILT, 1, NUM, 0);
set_hitbox_value(AT_DTILT, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DTILT, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_DTILT, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_DTILT, 4, HG_PARENT_HITBOX, 4);
set_hitbox_value(AT_DTILT, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 2, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 3, HG_WINDOW, 4);
set_hitbox_value(AT_DTILT, 4, HG_WINDOW, 4);
set_hitbox_value(AT_DTILT, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 2, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 3, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 4, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 1, HG_LIFETIME, 2);
set_hitbox_value(AT_DTILT, 2, HG_LIFETIME, 2);
set_hitbox_value(AT_DTILT, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_DTILT, 4, HG_LIFETIME, 3);
set_hitbox_value(AT_DTILT, 1, HG_BASE_KNOCKBACK, 2);
set_hitbox_value(AT_DTILT, 2, HG_BASE_KNOCKBACK, 2);
set_hitbox_value(AT_DTILT, 3, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DTILT, 4, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DTILT, 1, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_DTILT, 2, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_DTILT, 3, HG_KNOCKBACK_SCALING, 0.35);
set_hitbox_value(AT_DTILT, 4, HG_KNOCKBACK_SCALING, 0.50);
set_hitbox_value(AT_DTILT, 1, HG_ANGLE, 361);
set_hitbox_value(AT_DTILT, 2, HG_ANGLE, 90);
set_hitbox_value(AT_DTILT, 3, HG_ANGLE, 361);
set_hitbox_value(AT_DTILT, 4, HG_ANGLE, 100);
set_hitbox_value(AT_DTILT, 1, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DTILT, 2, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DTILT, 3, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DTILT, 4, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DTILT, 1, HG_DAMAGE, 3);
set_hitbox_value(AT_DTILT, 2, HG_DAMAGE, 5);
set_hitbox_value(AT_DTILT, 3, HG_DAMAGE, 3);
set_hitbox_value(AT_DTILT, 4, HG_DAMAGE, 5);
set_hitbox_value(AT_DTILT, 1, HG_WIDTH, 106);
set_hitbox_value(AT_DTILT, 1, HG_HEIGHT, 10);
set_hitbox_value(AT_DTILT, 2, HG_WIDTH, 52);
set_hitbox_value(AT_DTILT, 2, HG_HEIGHT, 24);
set_hitbox_value(AT_DTILT, 3, HG_WIDTH, 106);
set_hitbox_value(AT_DTILT, 3, HG_HEIGHT, 10);
set_hitbox_value(AT_DTILT, 4, HG_WIDTH, 52);
set_hitbox_value(AT_DTILT, 4, HG_HEIGHT, 24);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_X, 91);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_Y, -3);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_X, 166);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_Y, -10);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_X, 91);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_Y, -3);
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_X, 166);
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_Y, -10);
set_hitbox_value(AT_DTILT, 1, HG_EFFECT, 0);
set_hitbox_value(AT_DTILT, 2, HG_EFFECT, 0);
set_hitbox_value(AT_DTILT, 3, HG_EFFECT, 0);
set_hitbox_value(AT_DTILT, 4, HG_EFFECT, 0);
set_hitbox_value(AT_DTILT, 1, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_DTILT, 2, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_DTILT, 3, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_DTILT, 4, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_DTILT, 1, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DTILT, 2, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DTILT, 3, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DTILT, 4, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DTILT, 1, HG_TECHABLE, 0);
set_hitbox_value(AT_DTILT, 2, HG_TECHABLE, 0);
set_hitbox_value(AT_DTILT, 3, HG_TECHABLE, 0);
set_hitbox_value(AT_DTILT, 4, HG_TECHABLE, 0);
set_hitbox_value(AT_DTILT, 1, HG_PRIORITY, 1);
set_hitbox_value(AT_DTILT, 2, HG_PRIORITY, 6);
set_hitbox_value(AT_DTILT, 3, HG_PRIORITY, 1);
set_hitbox_value(AT_DTILT, 4, HG_PRIORITY, 6);
set_hitbox_value(AT_DTILT, 1, HG_BASE_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 2, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DTILT, 3, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_DTILT, 4, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DTILT, 1, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_DTILT, 2, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_DTILT, 3, HG_HITPAUSE_SCALING, 0.30);
set_hitbox_value(AT_DTILT, 4, HG_HITPAUSE_SCALING, 0.40);
set_hitbox_value(AT_DTILT, 1, HG_EXTRA_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 2, HG_EXTRA_HITPAUSE, 4);
set_hitbox_value(AT_DTILT, 3, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_DTILT, 4, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT, 0);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT, 132);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT, 0);
set_hitbox_value(AT_DTILT, 4, HG_VISUAL_EFFECT, 0);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT_X_OFFSET, 22);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT_X_OFFSET, 22);
set_hitbox_value(AT_DTILT, 4, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT_Y_OFFSET, -2);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT_Y_OFFSET, -2);
set_hitbox_value(AT_DTILT, 4, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DTILT, 1, HG_HIT_SFX, asset_get("sfx_blow_weak1"));
set_hitbox_value(AT_DTILT, 2, HG_HIT_SFX, asset_get("sfx_leafy_hit1"));
set_hitbox_value(AT_DTILT, 3, HG_HIT_SFX, asset_get("sfx_blow_weak2"));
set_hitbox_value(AT_DTILT, 4, HG_HIT_SFX, asset_get("sfx_leafy_hit2"));
set_hitbox_value(AT_DTILT, 1, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_DTILT, 2, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_DTILT, 3, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_DTILT, 4, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_DTILT, 1, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DTILT, 2, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DTILT, 3, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DTILT, 4, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DTILT, 1, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DTILT, 2, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DTILT, 3, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DTILT, 4, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DTILT, 1, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DTILT, 2, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DTILT, 3, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DTILT, 4, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_GROUP, 0);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_GROUP, 0);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DTILT, 4, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DTILT, 1, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DTILT, 2, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DTILT, 3, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DTILT, 4, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DTILT, 1, HG_SHAPE, 1);
set_hitbox_value(AT_DTILT, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DTILT, 3, HG_SHAPE, 1);
set_hitbox_value(AT_DTILT, 4, HG_SHAPE, 0);

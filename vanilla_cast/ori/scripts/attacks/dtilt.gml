set_attack_value(AT_DTILT, AG_CATEGORY, 3);

set_attack_value(AT_DTILT, AG_SPRITE, sprite_get("dtilt"));

set_attack_value(AT_DTILT, AG_HURTBOX_SPRITE, sprite_get("dtilt_hurt"));

set_attack_value(AT_DTILT, AG_NUM_WINDOWS, 6);
set_window_value(AT_DTILT, 1, AG_WINDOW_LENGTH, 7);
set_window_value(AT_DTILT, 1, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX, asset_get("sfx_ori_dtilt_perform"));
set_window_value(AT_DTILT, 1, AG_WINDOW_SFX_FRAME, 1);
set_window_value(AT_DTILT, 1, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DTILT, 1, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_LENGTH, 12);
set_window_value(AT_DTILT, 2, AG_WINDOW_HSPEED, 10);
set_window_value(AT_DTILT, 2, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_SFX, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DTILT, 2, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_LENGTH, 16);
set_window_value(AT_DTILT, 3, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_SFX, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DTILT, 3, AG_WINDOW_HAS_WHIFFLAG, 6);
set_window_value(AT_DTILT, 3, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_LENGTH, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_SFX, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DTILT, 4, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_LENGTH, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_SFX, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DTILT, 5, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DTILT, 6, AG_WINDOW_LENGTH, 0);
set_window_value(AT_DTILT, 6, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DTILT, 6, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DTILT, 6, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DTILT, 6, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DTILT, 6, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DTILT, 6, AG_WINDOW_SFX, 0);
set_window_value(AT_DTILT, 6, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DTILT, 6, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DTILT, 6, AG_WINDOW_INVINCIBILITY, 0);
//ERROR_NO_SETTER_FOUND(AT_DTILT, HAS_COOLDOWN, 0);
//ERROR_NO_SETTER_FOUND(AT_DTILT, COOLDOWN, 0);
set_num_hitboxes(AT_DTILT, 3);
//ERROR_NO_SETTER_FOUND(AT_DTILT, NUM_UNIQUE_HITBOXES, 2);
//set_hitbox_value(AT_DTILT, 1, NUM, 1);
set_hitbox_value(AT_DTILT, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DTILT, 2, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DTILT, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_DTILT, 1, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 2, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 3, HG_WINDOW, 2);
set_hitbox_value(AT_DTILT, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DTILT, 2, HG_WINDOW_CREATION_FRAME, 3);
set_hitbox_value(AT_DTILT, 3, HG_WINDOW_CREATION_FRAME, 6);
set_hitbox_value(AT_DTILT, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_DTILT, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_DTILT, 3, HG_LIFETIME, 6);
set_hitbox_value(AT_DTILT, 1, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DTILT, 2, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_DTILT, 3, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_DTILT, 1, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_DTILT, 2, HG_KNOCKBACK_SCALING, 0);
set_hitbox_value(AT_DTILT, 3, HG_KNOCKBACK_SCALING, 0.40);
set_hitbox_value(AT_DTILT, 1, HG_ANGLE, 10);
set_hitbox_value(AT_DTILT, 2, HG_ANGLE, 10);
set_hitbox_value(AT_DTILT, 3, HG_ANGLE, 70);
set_hitbox_value(AT_DTILT, 1, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DTILT, 2, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DTILT, 3, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DTILT, 1, HG_DAMAGE, 2);
set_hitbox_value(AT_DTILT, 2, HG_DAMAGE, 2);
set_hitbox_value(AT_DTILT, 3, HG_DAMAGE, 4);
set_hitbox_value(AT_DTILT, 1, HG_WIDTH, 76);
set_hitbox_value(AT_DTILT, 2, HG_WIDTH, 76);
set_hitbox_value(AT_DTILT, 1, HG_HEIGHT, 51);
set_hitbox_value(AT_DTILT, 2, HG_HEIGHT, 51);
set_hitbox_value(AT_DTILT, 3, HG_WIDTH, 76);
set_hitbox_value(AT_DTILT, 3, HG_HEIGHT, 51);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_X, -2);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_Y, -18);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_X, -2);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_Y, -18);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_X, -2);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_Y, -18);
set_hitbox_value(AT_DTILT, 1, HG_EFFECT, 0);
set_hitbox_value(AT_DTILT, 2, HG_EFFECT, 0);
set_hitbox_value(AT_DTILT, 3, HG_EFFECT, 0);
set_hitbox_value(AT_DTILT, 1, HG_SDI_MULTIPLIER, 0);
set_hitbox_value(AT_DTILT, 2, HG_SDI_MULTIPLIER, 0);
set_hitbox_value(AT_DTILT, 3, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_DTILT, 1, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DTILT, 2, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DTILT, 3, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DTILT, 1, HG_TECHABLE, 1);
set_hitbox_value(AT_DTILT, 2, HG_TECHABLE, 1);
set_hitbox_value(AT_DTILT, 3, HG_TECHABLE, 0);
set_hitbox_value(AT_DTILT, 1, HG_PRIORITY, 5);
set_hitbox_value(AT_DTILT, 2, HG_PRIORITY, 5);
set_hitbox_value(AT_DTILT, 3, HG_PRIORITY, 1);
set_hitbox_value(AT_DTILT, 1, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DTILT, 2, HG_BASE_HITPAUSE, 6);
set_hitbox_value(AT_DTILT, 3, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_DTILT, 1, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_DTILT, 2, HG_HITPAUSE_SCALING, 0);
set_hitbox_value(AT_DTILT, 3, HG_HITPAUSE_SCALING, 0.35);
set_hitbox_value(AT_DTILT, 1, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_DTILT, 2, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_DTILT, 3, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT, 109);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT, 109);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT, 109);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT_X_OFFSET, 0);
set_hitbox_value(AT_DTILT, 1, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DTILT, 2, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DTILT, 3, HG_VISUAL_EFFECT_Y_OFFSET, 0);
set_hitbox_value(AT_DTILT, 1, HG_HIT_SFX, asset_get("sfx_ori_energyhit_weak"));
set_hitbox_value(AT_DTILT, 2, HG_HIT_SFX, asset_get("sfx_ori_energyhit_weak"));
set_hitbox_value(AT_DTILT, 3, HG_HIT_SFX, asset_get("sfx_ori_energyhit_medium"));
set_hitbox_value(AT_DTILT, 1, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_DTILT, 2, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_DTILT, 3, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_DTILT, 1, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DTILT, 2, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DTILT, 3, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DTILT, 1, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DTILT, 2, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DTILT, 3, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DTILT, 1, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DTILT, 2, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DTILT, 3, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DTILT, 1, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DTILT, 2, HG_HITBOX_GROUP, 2);
set_hitbox_value(AT_DTILT, 3, HG_HITBOX_GROUP, 0);
set_hitbox_value(AT_DTILT, 1, HG_FORCE_FLINCH, 2);
set_hitbox_value(AT_DTILT, 2, HG_FORCE_FLINCH, 2);
set_hitbox_value(AT_DTILT, 3, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DTILT, 1, HG_SHAPE, 0);
set_hitbox_value(AT_DTILT, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DTILT, 3, HG_SHAPE, 0);

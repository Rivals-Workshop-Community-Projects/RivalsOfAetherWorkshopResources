set_attack_value(AT_DSTRONG, AG_CATEGORY, 3);

set_attack_value(AT_DSTRONG, AG_SPRITE, sprite_get("dstrong"));

set_attack_value(AT_DSTRONG, AG_HURTBOX_SPRITE, sprite_get("dstrong_hurt"));

set_attack_value(AT_DSTRONG, AG_NUM_WINDOWS, 6);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_SFX, 0);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DSTRONG, 1, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_LENGTH, 4);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_SFX, asset_get("sfx_clairen_swing_med"));
set_window_value(AT_DSTRONG, 2, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DSTRONG, 2, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_SFX, 0);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DSTRONG, 3, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_LENGTH, 9);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_SFX, asset_get("sfx_clairen_swing_strong"));
set_window_value(AT_DSTRONG, 4, AG_WINDOW_SFX_FRAME, 3);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DSTRONG, 4, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DSTRONG, 5, AG_WINDOW_LENGTH, 3);
set_window_value(AT_DSTRONG, 5, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DSTRONG, 5, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DSTRONG, 5, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 5, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 5, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DSTRONG, 5, AG_WINDOW_SFX, 0);
set_window_value(AT_DSTRONG, 5, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DSTRONG, 5, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_DSTRONG, 5, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_DSTRONG, 6, AG_WINDOW_LENGTH, 16);
set_window_value(AT_DSTRONG, 6, AG_WINDOW_HSPEED, 0);
set_window_value(AT_DSTRONG, 6, AG_WINDOW_VSPEED, 0);
set_window_value(AT_DSTRONG, 6, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 6, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_DSTRONG, 6, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_DSTRONG, 6, AG_WINDOW_SFX, 0);
set_window_value(AT_DSTRONG, 6, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_DSTRONG, 6, AG_WINDOW_HAS_WHIFFLAG, 6);
set_window_value(AT_DSTRONG, 6, AG_WINDOW_INVINCIBILITY, 0);
//ERROR_NO_SETTER_FOUND(AT_DSTRONG, HAS_COOLDOWN, 0);
//ERROR_NO_SETTER_FOUND(AT_DSTRONG, COOLDOWN, 0);
set_num_hitboxes(AT_DSTRONG, 5);
//ERROR_NO_SETTER_FOUND(AT_DSTRONG, NUM_UNIQUE_HITBOXES, 5);
//set_hitbox_value(AT_DSTRONG, 1, NUM, 0);
set_hitbox_value(AT_DSTRONG, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_DSTRONG, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_DSTRONG, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_DSTRONG, 4, HG_PARENT_HITBOX, 4);
set_hitbox_value(AT_DSTRONG, 5, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_DSTRONG, 1, HG_WINDOW, 3);
set_hitbox_value(AT_DSTRONG, 2, HG_WINDOW, 3);
set_hitbox_value(AT_DSTRONG, 3, HG_WINDOW, 3);
set_hitbox_value(AT_DSTRONG, 4, HG_WINDOW, 5);
set_hitbox_value(AT_DSTRONG, 5, HG_WINDOW, 5);
set_hitbox_value(AT_DSTRONG, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DSTRONG, 2, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DSTRONG, 3, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DSTRONG, 4, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DSTRONG, 5, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_DSTRONG, 1, HG_LIFETIME, 3);
set_hitbox_value(AT_DSTRONG, 2, HG_LIFETIME, 3);
set_hitbox_value(AT_DSTRONG, 3, HG_LIFETIME, 3);
set_hitbox_value(AT_DSTRONG, 4, HG_LIFETIME, 3);
set_hitbox_value(AT_DSTRONG, 5, HG_LIFETIME, 3);
set_hitbox_value(AT_DSTRONG, 1, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_DSTRONG, 2, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_DSTRONG, 3, HG_BASE_KNOCKBACK, 8);
set_hitbox_value(AT_DSTRONG, 4, HG_BASE_KNOCKBACK, 7);
set_hitbox_value(AT_DSTRONG, 5, HG_BASE_KNOCKBACK, 9);
set_hitbox_value(AT_DSTRONG, 1, HG_KNOCKBACK_SCALING, 0.90);
set_hitbox_value(AT_DSTRONG, 2, HG_KNOCKBACK_SCALING, 1.20);
set_hitbox_value(AT_DSTRONG, 3, HG_KNOCKBACK_SCALING, 1.20);
set_hitbox_value(AT_DSTRONG, 4, HG_KNOCKBACK_SCALING, 0.90);
set_hitbox_value(AT_DSTRONG, 5, HG_KNOCKBACK_SCALING, 1.25);
set_hitbox_value(AT_DSTRONG, 1, HG_ANGLE, 130);
set_hitbox_value(AT_DSTRONG, 2, HG_ANGLE, 110);
set_hitbox_value(AT_DSTRONG, 3, HG_ANGLE, 110);
set_hitbox_value(AT_DSTRONG, 4, HG_ANGLE, 50);
set_hitbox_value(AT_DSTRONG, 5, HG_ANGLE, 70);
set_hitbox_value(AT_DSTRONG, 1, HG_ANGLE_FLIPPER, 6);
set_hitbox_value(AT_DSTRONG, 2, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DSTRONG, 3, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DSTRONG, 4, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DSTRONG, 5, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_DSTRONG, 1, HG_DAMAGE, 9);
set_hitbox_value(AT_DSTRONG, 2, HG_DAMAGE, 13);
set_hitbox_value(AT_DSTRONG, 3, HG_DAMAGE, 13);
set_hitbox_value(AT_DSTRONG, 4, HG_DAMAGE, 9);
set_hitbox_value(AT_DSTRONG, 5, HG_DAMAGE, 13);
set_hitbox_value(AT_DSTRONG, 1, HG_WIDTH, 64);
set_hitbox_value(AT_DSTRONG, 1, HG_HEIGHT, 50);
set_hitbox_value(AT_DSTRONG, 2, HG_WIDTH, 66);
set_hitbox_value(AT_DSTRONG, 2, HG_HEIGHT, 62);
set_hitbox_value(AT_DSTRONG, 3, HG_WIDTH, 36);
set_hitbox_value(AT_DSTRONG, 3, HG_HEIGHT, 36);
set_hitbox_value(AT_DSTRONG, 4, HG_WIDTH, 62);
set_hitbox_value(AT_DSTRONG, 4, HG_HEIGHT, 36);
set_hitbox_value(AT_DSTRONG, 5, HG_WIDTH, 38);
set_hitbox_value(AT_DSTRONG, 5, HG_HEIGHT, 28);
set_hitbox_value(AT_DSTRONG, 1, HG_HITBOX_X, -32);
set_hitbox_value(AT_DSTRONG, 1, HG_HITBOX_Y, -23);
set_hitbox_value(AT_DSTRONG, 2, HG_HITBOX_X, -41);
set_hitbox_value(AT_DSTRONG, 2, HG_HITBOX_Y, -29);
set_hitbox_value(AT_DSTRONG, 3, HG_HITBOX_X, -72);
set_hitbox_value(AT_DSTRONG, 3, HG_HITBOX_Y, -16);
set_hitbox_value(AT_DSTRONG, 4, HG_HITBOX_X, 55);
set_hitbox_value(AT_DSTRONG, 4, HG_HITBOX_Y, -16);
set_hitbox_value(AT_DSTRONG, 5, HG_HITBOX_X, 100);
set_hitbox_value(AT_DSTRONG, 5, HG_HITBOX_Y, -6);
set_hitbox_value(AT_DSTRONG, 1, HG_EFFECT, 0);
set_hitbox_value(AT_DSTRONG, 2, HG_EFFECT, 11);
set_hitbox_value(AT_DSTRONG, 3, HG_EFFECT, 11);
set_hitbox_value(AT_DSTRONG, 4, HG_EFFECT, 0);
set_hitbox_value(AT_DSTRONG, 5, HG_EFFECT, 11);
set_hitbox_value(AT_DSTRONG, 1, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_DSTRONG, 2, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_DSTRONG, 3, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_DSTRONG, 4, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_DSTRONG, 5, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_DSTRONG, 1, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DSTRONG, 2, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DSTRONG, 3, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DSTRONG, 4, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DSTRONG, 5, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_DSTRONG, 1, HG_TECHABLE, 0);
set_hitbox_value(AT_DSTRONG, 2, HG_TECHABLE, 0);
set_hitbox_value(AT_DSTRONG, 3, HG_TECHABLE, 0);
set_hitbox_value(AT_DSTRONG, 4, HG_TECHABLE, 0);
set_hitbox_value(AT_DSTRONG, 5, HG_TECHABLE, 0);
set_hitbox_value(AT_DSTRONG, 1, HG_PRIORITY, 6);
set_hitbox_value(AT_DSTRONG, 2, HG_PRIORITY, 4);
set_hitbox_value(AT_DSTRONG, 3, HG_PRIORITY, 3);
set_hitbox_value(AT_DSTRONG, 4, HG_PRIORITY, 6);
set_hitbox_value(AT_DSTRONG, 5, HG_PRIORITY, 3);
set_hitbox_value(AT_DSTRONG, 1, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_DSTRONG, 2, HG_BASE_HITPAUSE, 12);
set_hitbox_value(AT_DSTRONG, 3, HG_BASE_HITPAUSE, 12);
set_hitbox_value(AT_DSTRONG, 4, HG_BASE_HITPAUSE, 8);
set_hitbox_value(AT_DSTRONG, 5, HG_BASE_HITPAUSE, 12);
set_hitbox_value(AT_DSTRONG, 1, HG_HITPAUSE_SCALING, 0.70);
set_hitbox_value(AT_DSTRONG, 2, HG_HITPAUSE_SCALING, 0.80);
set_hitbox_value(AT_DSTRONG, 3, HG_HITPAUSE_SCALING, 0.80);
set_hitbox_value(AT_DSTRONG, 4, HG_HITPAUSE_SCALING, 0.70);
set_hitbox_value(AT_DSTRONG, 5, HG_HITPAUSE_SCALING, 0.80);
set_hitbox_value(AT_DSTRONG, 1, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_DSTRONG, 2, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_DSTRONG, 3, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_DSTRONG, 4, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_DSTRONG, 5, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_DSTRONG, 1, HG_VISUAL_EFFECT, 123);
set_hitbox_value(AT_DSTRONG, 2, HG_VISUAL_EFFECT, 127);
set_hitbox_value(AT_DSTRONG, 3, HG_VISUAL_EFFECT, 127);
set_hitbox_value(AT_DSTRONG, 4, HG_VISUAL_EFFECT, 123);
set_hitbox_value(AT_DSTRONG, 5, HG_VISUAL_EFFECT, 127);
set_hitbox_value(AT_DSTRONG, 1, HG_VISUAL_EFFECT_X_OFFSET, -23);
set_hitbox_value(AT_DSTRONG, 2, HG_VISUAL_EFFECT_X_OFFSET, -20);
set_hitbox_value(AT_DSTRONG, 3, HG_VISUAL_EFFECT_X_OFFSET, -6);
set_hitbox_value(AT_DSTRONG, 4, HG_VISUAL_EFFECT_X_OFFSET, 18);
set_hitbox_value(AT_DSTRONG, 5, HG_VISUAL_EFFECT_X_OFFSET, 6);
set_hitbox_value(AT_DSTRONG, 1, HG_VISUAL_EFFECT_Y_OFFSET, -4);
set_hitbox_value(AT_DSTRONG, 2, HG_VISUAL_EFFECT_Y_OFFSET, -10);
set_hitbox_value(AT_DSTRONG, 3, HG_VISUAL_EFFECT_Y_OFFSET, 6);
set_hitbox_value(AT_DSTRONG, 4, HG_VISUAL_EFFECT_Y_OFFSET, 2);
set_hitbox_value(AT_DSTRONG, 5, HG_VISUAL_EFFECT_Y_OFFSET, -2);
set_hitbox_value(AT_DSTRONG, 1, HG_HIT_SFX, asset_get("sfx_clairen_hit_strong"));
set_hitbox_value(AT_DSTRONG, 2, HG_HIT_SFX, asset_get("sfx_clairen_tip_strong"));
set_hitbox_value(AT_DSTRONG, 3, HG_HIT_SFX, asset_get("sfx_clairen_tip_strong"));
set_hitbox_value(AT_DSTRONG, 4, HG_HIT_SFX, asset_get("sfx_clairen_hit_strong"));
set_hitbox_value(AT_DSTRONG, 5, HG_HIT_SFX, asset_get("sfx_clairen_tip_strong"));
set_hitbox_value(AT_DSTRONG, 1, HG_EXTRA_CAMERA_SHAKE, 2);
set_hitbox_value(AT_DSTRONG, 2, HG_EXTRA_CAMERA_SHAKE, 2);
set_hitbox_value(AT_DSTRONG, 3, HG_EXTRA_CAMERA_SHAKE, 2);
set_hitbox_value(AT_DSTRONG, 4, HG_EXTRA_CAMERA_SHAKE, 2);
set_hitbox_value(AT_DSTRONG, 5, HG_EXTRA_CAMERA_SHAKE, 2);
set_hitbox_value(AT_DSTRONG, 1, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DSTRONG, 2, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DSTRONG, 3, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DSTRONG, 4, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DSTRONG, 5, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_DSTRONG, 1, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DSTRONG, 2, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DSTRONG, 3, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DSTRONG, 4, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DSTRONG, 5, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_DSTRONG, 1, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DSTRONG, 2, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DSTRONG, 3, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DSTRONG, 4, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DSTRONG, 5, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_DSTRONG, 1, HG_HITBOX_GROUP, 0);
set_hitbox_value(AT_DSTRONG, 2, HG_HITBOX_GROUP, 0);
set_hitbox_value(AT_DSTRONG, 3, HG_HITBOX_GROUP, 0);
set_hitbox_value(AT_DSTRONG, 4, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DSTRONG, 5, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_DSTRONG, 1, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DSTRONG, 2, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DSTRONG, 3, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DSTRONG, 4, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DSTRONG, 5, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_DSTRONG, 1, HG_SHAPE, 0);
set_hitbox_value(AT_DSTRONG, 2, HG_SHAPE, 0);
set_hitbox_value(AT_DSTRONG, 3, HG_SHAPE, 0);
set_hitbox_value(AT_DSTRONG, 4, HG_SHAPE, 0);
set_hitbox_value(AT_DSTRONG, 5, HG_SHAPE, 0);

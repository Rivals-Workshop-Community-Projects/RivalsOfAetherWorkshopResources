set_attack_value(AT_FAIR, AG_CATEGORY, 1);

set_attack_value(AT_FAIR, AG_SPRITE, sprite_get("fair"));

set_attack_value(AT_FAIR, AG_HURTBOX_SPRITE, sprite_get("fair_hurt"));

set_attack_value(AT_FAIR, AG_NUM_WINDOWS, 5);
set_window_value(AT_FAIR, 1, AG_WINDOW_LENGTH, 6);
set_window_value(AT_FAIR, 1, AG_WINDOW_HSPEED, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FAIR, 1, AG_WINDOW_SFX, asset_get("sfx_clairen_swing_weak"));
set_window_value(AT_FAIR, 1, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_FAIR, 1, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_LENGTH, 2);
set_window_value(AT_FAIR, 2, AG_WINDOW_HSPEED, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_SFX, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_FAIR, 2, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_LENGTH, 7);
set_window_value(AT_FAIR, 3, AG_WINDOW_HSPEED, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_HAS_SFX, 1);
set_window_value(AT_FAIR, 3, AG_WINDOW_SFX, asset_get("sfx_clairen_swing_med"));
set_window_value(AT_FAIR, 3, AG_WINDOW_SFX_FRAME, 3);
set_window_value(AT_FAIR, 3, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_FAIR, 3, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_LENGTH, 2);
set_window_value(AT_FAIR, 4, AG_WINDOW_HSPEED, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_SFX, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_HAS_WHIFFLAG, 0);
set_window_value(AT_FAIR, 4, AG_WINDOW_INVINCIBILITY, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_LENGTH, 14);
set_window_value(AT_FAIR, 5, AG_WINDOW_HSPEED, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_VSPEED, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_HSPEED_TYPE, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_VSPEED_TYPE, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_HAS_SFX, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_SFX, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_SFX_FRAME, 0);
set_window_value(AT_FAIR, 5, AG_WINDOW_HAS_WHIFFLAG, 5);
set_window_value(AT_FAIR, 5, AG_WINDOW_INVINCIBILITY, 0);
//ERROR_NO_SETTER_FOUND(AT_FAIR, HAS_COOLDOWN, 0);
//ERROR_NO_SETTER_FOUND(AT_FAIR, COOLDOWN, 0);
//ERROR_NO_SETTER_FOUND(AT_FAIR, HAS_WHIFF_LANDING, 0);
//ERROR_NO_SETTER_FOUND(AT_FAIR, AUTOCANCEL_FRAME, 24);
//ERROR_NO_SETTER_FOUND(AT_FAIR, LANDING_LAG, 8);
set_num_hitboxes(AT_FAIR, 7);
//ERROR_NO_SETTER_FOUND(AT_FAIR, NUM_UNIQUE_HITBOXES, 7);
//set_hitbox_value(AT_FAIR, 1, NUM, 0);
set_hitbox_value(AT_FAIR, 1, HG_PARENT_HITBOX, 1);
set_hitbox_value(AT_FAIR, 2, HG_PARENT_HITBOX, 2);
set_hitbox_value(AT_FAIR, 3, HG_PARENT_HITBOX, 3);
set_hitbox_value(AT_FAIR, 4, HG_PARENT_HITBOX, 4);
set_hitbox_value(AT_FAIR, 5, HG_PARENT_HITBOX, 5);
set_hitbox_value(AT_FAIR, 6, HG_PARENT_HITBOX, 6);
set_hitbox_value(AT_FAIR, 7, HG_PARENT_HITBOX, 7);
set_hitbox_value(AT_FAIR, 1, HG_WINDOW, 2);
set_hitbox_value(AT_FAIR, 2, HG_WINDOW, 2);
set_hitbox_value(AT_FAIR, 3, HG_WINDOW, 4);
set_hitbox_value(AT_FAIR, 4, HG_WINDOW, 4);
set_hitbox_value(AT_FAIR, 5, HG_WINDOW, 2);
set_hitbox_value(AT_FAIR, 6, HG_WINDOW, 4);
set_hitbox_value(AT_FAIR, 7, HG_WINDOW, 4);
set_hitbox_value(AT_FAIR, 1, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FAIR, 2, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FAIR, 3, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FAIR, 4, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FAIR, 5, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FAIR, 6, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FAIR, 7, HG_WINDOW_CREATION_FRAME, 0);
set_hitbox_value(AT_FAIR, 1, HG_LIFETIME, 2);
set_hitbox_value(AT_FAIR, 2, HG_LIFETIME, 2);
set_hitbox_value(AT_FAIR, 3, HG_LIFETIME, 2);
set_hitbox_value(AT_FAIR, 4, HG_LIFETIME, 2);
set_hitbox_value(AT_FAIR, 5, HG_LIFETIME, 2);
set_hitbox_value(AT_FAIR, 6, HG_LIFETIME, 2);
set_hitbox_value(AT_FAIR, 7, HG_LIFETIME, 2);
set_hitbox_value(AT_FAIR, 1, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_FAIR, 2, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_FAIR, 3, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_FAIR, 4, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_FAIR, 5, HG_BASE_KNOCKBACK, 3);
set_hitbox_value(AT_FAIR, 6, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_FAIR, 7, HG_BASE_KNOCKBACK, 6);
set_hitbox_value(AT_FAIR, 1, HG_KNOCKBACK_SCALING, 0.10);
set_hitbox_value(AT_FAIR, 2, HG_KNOCKBACK_SCALING, 0.80);
set_hitbox_value(AT_FAIR, 3, HG_KNOCKBACK_SCALING, 0.35);
set_hitbox_value(AT_FAIR, 4, HG_KNOCKBACK_SCALING, 0.80);
set_hitbox_value(AT_FAIR, 5, HG_KNOCKBACK_SCALING, 0.10);
set_hitbox_value(AT_FAIR, 6, HG_KNOCKBACK_SCALING, 0.40);
set_hitbox_value(AT_FAIR, 7, HG_KNOCKBACK_SCALING, 0.40);
set_hitbox_value(AT_FAIR, 1, HG_ANGLE, 82);
set_hitbox_value(AT_FAIR, 2, HG_ANGLE, 40);
set_hitbox_value(AT_FAIR, 3, HG_ANGLE, 361);
set_hitbox_value(AT_FAIR, 4, HG_ANGLE, 40);
set_hitbox_value(AT_FAIR, 5, HG_ANGLE, 82);
set_hitbox_value(AT_FAIR, 6, HG_ANGLE, 60);
set_hitbox_value(AT_FAIR, 7, HG_ANGLE, 60);
set_hitbox_value(AT_FAIR, 1, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FAIR, 2, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FAIR, 3, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FAIR, 4, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FAIR, 5, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FAIR, 6, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FAIR, 7, HG_ANGLE_FLIPPER, 0);
set_hitbox_value(AT_FAIR, 1, HG_DAMAGE, 3);
set_hitbox_value(AT_FAIR, 2, HG_DAMAGE, 7);
set_hitbox_value(AT_FAIR, 3, HG_DAMAGE, 4);
set_hitbox_value(AT_FAIR, 4, HG_DAMAGE, 7);
set_hitbox_value(AT_FAIR, 5, HG_DAMAGE, 3);
set_hitbox_value(AT_FAIR, 6, HG_DAMAGE, 4);
set_hitbox_value(AT_FAIR, 7, HG_DAMAGE, 4);
set_hitbox_value(AT_FAIR, 1, HG_WIDTH, 44);
set_hitbox_value(AT_FAIR, 1, HG_HEIGHT, 52);
set_hitbox_value(AT_FAIR, 2, HG_WIDTH, 60);
set_hitbox_value(AT_FAIR, 2, HG_HEIGHT, 50);
set_hitbox_value(AT_FAIR, 3, HG_WIDTH, 50);
set_hitbox_value(AT_FAIR, 3, HG_HEIGHT, 58);
set_hitbox_value(AT_FAIR, 4, HG_WIDTH, 60);
set_hitbox_value(AT_FAIR, 4, HG_HEIGHT, 50);
set_hitbox_value(AT_FAIR, 5, HG_WIDTH, 34);
set_hitbox_value(AT_FAIR, 5, HG_HEIGHT, 20);
set_hitbox_value(AT_FAIR, 6, HG_WIDTH, 35);
set_hitbox_value(AT_FAIR, 6, HG_HEIGHT, 20);
set_hitbox_value(AT_FAIR, 7, HG_WIDTH, 35);
set_hitbox_value(AT_FAIR, 7, HG_HEIGHT, 10);
set_hitbox_value(AT_FAIR, 1, HG_HITBOX_X, 37);
set_hitbox_value(AT_FAIR, 1, HG_HITBOX_Y, -25);
set_hitbox_value(AT_FAIR, 2, HG_HITBOX_X, 48);
set_hitbox_value(AT_FAIR, 2, HG_HITBOX_Y, -26);
set_hitbox_value(AT_FAIR, 3, HG_HITBOX_X, 45);
set_hitbox_value(AT_FAIR, 3, HG_HITBOX_Y, -45);
set_hitbox_value(AT_FAIR, 4, HG_HITBOX_X, 62);
set_hitbox_value(AT_FAIR, 4, HG_HITBOX_Y, -45);
set_hitbox_value(AT_FAIR, 5, HG_HITBOX_X, 12);
set_hitbox_value(AT_FAIR, 5, HG_HITBOX_Y, 2);
set_hitbox_value(AT_FAIR, 6, HG_HITBOX_X, 22);
set_hitbox_value(AT_FAIR, 6, HG_HITBOX_Y, -76);
set_hitbox_value(AT_FAIR, 7, HG_HITBOX_X, 22);
set_hitbox_value(AT_FAIR, 7, HG_HITBOX_Y, -12);
set_hitbox_value(AT_FAIR, 1, HG_EFFECT, 0);
set_hitbox_value(AT_FAIR, 2, HG_EFFECT, 11);
set_hitbox_value(AT_FAIR, 3, HG_EFFECT, 0);
set_hitbox_value(AT_FAIR, 4, HG_EFFECT, 11);
set_hitbox_value(AT_FAIR, 5, HG_EFFECT, 0);
set_hitbox_value(AT_FAIR, 6, HG_EFFECT, 0);
set_hitbox_value(AT_FAIR, 7, HG_EFFECT, 0);
set_hitbox_value(AT_FAIR, 1, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_FAIR, 2, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_FAIR, 3, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_FAIR, 4, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_FAIR, 5, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_FAIR, 6, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_FAIR, 7, HG_SDI_MULTIPLIER, 1);
set_hitbox_value(AT_FAIR, 1, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_FAIR, 2, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_FAIR, 3, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_FAIR, 4, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_FAIR, 5, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_FAIR, 6, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_FAIR, 7, HG_HITSTUN_MULTIPLIER, 0);
set_hitbox_value(AT_FAIR, 1, HG_TECHABLE, 0);
set_hitbox_value(AT_FAIR, 2, HG_TECHABLE, 0);
set_hitbox_value(AT_FAIR, 3, HG_TECHABLE, 0);
set_hitbox_value(AT_FAIR, 4, HG_TECHABLE, 0);
set_hitbox_value(AT_FAIR, 5, HG_TECHABLE, 0);
set_hitbox_value(AT_FAIR, 6, HG_TECHABLE, 0);
set_hitbox_value(AT_FAIR, 7, HG_TECHABLE, 0);
set_hitbox_value(AT_FAIR, 1, HG_PRIORITY, 6);
set_hitbox_value(AT_FAIR, 2, HG_PRIORITY, 3);
set_hitbox_value(AT_FAIR, 3, HG_PRIORITY, 6);
set_hitbox_value(AT_FAIR, 4, HG_PRIORITY, 3);
set_hitbox_value(AT_FAIR, 5, HG_PRIORITY, 5);
set_hitbox_value(AT_FAIR, 6, HG_PRIORITY, 5);
set_hitbox_value(AT_FAIR, 7, HG_PRIORITY, 5);
set_hitbox_value(AT_FAIR, 1, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_FAIR, 2, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_FAIR, 3, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_FAIR, 4, HG_BASE_HITPAUSE, 10);
set_hitbox_value(AT_FAIR, 5, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_FAIR, 6, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_FAIR, 7, HG_BASE_HITPAUSE, 5);
set_hitbox_value(AT_FAIR, 1, HG_HITPAUSE_SCALING, 0.10);
set_hitbox_value(AT_FAIR, 2, HG_HITPAUSE_SCALING, 0.80);
set_hitbox_value(AT_FAIR, 3, HG_HITPAUSE_SCALING, 0.20);
set_hitbox_value(AT_FAIR, 4, HG_HITPAUSE_SCALING, 0.80);
set_hitbox_value(AT_FAIR, 5, HG_HITPAUSE_SCALING, 0.10);
set_hitbox_value(AT_FAIR, 6, HG_HITPAUSE_SCALING, 0.20);
set_hitbox_value(AT_FAIR, 7, HG_HITPAUSE_SCALING, 0.20);
set_hitbox_value(AT_FAIR, 1, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_FAIR, 2, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_FAIR, 3, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_FAIR, 4, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_FAIR, 5, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_FAIR, 6, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_FAIR, 7, HG_EXTRA_HITPAUSE, 0);
set_hitbox_value(AT_FAIR, 1, HG_VISUAL_EFFECT, 124);
set_hitbox_value(AT_FAIR, 2, HG_VISUAL_EFFECT, 125);
set_hitbox_value(AT_FAIR, 3, HG_VISUAL_EFFECT, 123);
set_hitbox_value(AT_FAIR, 4, HG_VISUAL_EFFECT, 125);
set_hitbox_value(AT_FAIR, 5, HG_VISUAL_EFFECT, 124);
set_hitbox_value(AT_FAIR, 6, HG_VISUAL_EFFECT, 123);
set_hitbox_value(AT_FAIR, 7, HG_VISUAL_EFFECT, 123);
set_hitbox_value(AT_FAIR, 1, HG_VISUAL_EFFECT_X_OFFSET, 24);
set_hitbox_value(AT_FAIR, 2, HG_VISUAL_EFFECT_X_OFFSET, 33);
set_hitbox_value(AT_FAIR, 3, HG_VISUAL_EFFECT_X_OFFSET, 34);
set_hitbox_value(AT_FAIR, 4, HG_VISUAL_EFFECT_X_OFFSET, 44);
set_hitbox_value(AT_FAIR, 5, HG_VISUAL_EFFECT_X_OFFSET, 24);
set_hitbox_value(AT_FAIR, 6, HG_VISUAL_EFFECT_X_OFFSET, 34);
set_hitbox_value(AT_FAIR, 7, HG_VISUAL_EFFECT_X_OFFSET, 34);
set_hitbox_value(AT_FAIR, 1, HG_VISUAL_EFFECT_Y_OFFSET, -2);
set_hitbox_value(AT_FAIR, 2, HG_VISUAL_EFFECT_Y_OFFSET, -5);
set_hitbox_value(AT_FAIR, 3, HG_VISUAL_EFFECT_Y_OFFSET, -6);
set_hitbox_value(AT_FAIR, 4, HG_VISUAL_EFFECT_Y_OFFSET, -4);
set_hitbox_value(AT_FAIR, 5, HG_VISUAL_EFFECT_Y_OFFSET, -2);
set_hitbox_value(AT_FAIR, 6, HG_VISUAL_EFFECT_Y_OFFSET, -6);
set_hitbox_value(AT_FAIR, 7, HG_VISUAL_EFFECT_Y_OFFSET, -6);
set_hitbox_value(AT_FAIR, 1, HG_HIT_SFX, asset_get("sfx_clairen_hit_weak"));
set_hitbox_value(AT_FAIR, 2, HG_HIT_SFX, asset_get("sfx_clairen_tip_med"));
set_hitbox_value(AT_FAIR, 3, HG_HIT_SFX, asset_get("sfx_clairen_hit_med"));
set_hitbox_value(AT_FAIR, 4, HG_HIT_SFX, asset_get("sfx_clairen_tip_med"));
set_hitbox_value(AT_FAIR, 5, HG_HIT_SFX, asset_get("sfx_clairen_hit_weak"));
set_hitbox_value(AT_FAIR, 6, HG_HIT_SFX, asset_get("sfx_clairen_hit_med"));
set_hitbox_value(AT_FAIR, 7, HG_HIT_SFX, asset_get("sfx_clairen_hit_med"));
set_hitbox_value(AT_FAIR, 1, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_FAIR, 2, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_FAIR, 3, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_FAIR, 4, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_FAIR, 5, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_FAIR, 6, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_FAIR, 7, HG_EXTRA_CAMERA_SHAKE, 0);
set_hitbox_value(AT_FAIR, 1, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_FAIR, 2, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_FAIR, 3, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_FAIR, 4, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_FAIR, 5, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_FAIR, 6, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_FAIR, 7, HG_IGNORES_PROJECTILES, 1);
set_hitbox_value(AT_FAIR, 1, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_FAIR, 2, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_FAIR, 3, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_FAIR, 4, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_FAIR, 5, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_FAIR, 6, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_FAIR, 7, HG_HIT_LOCKOUT, 0);
set_hitbox_value(AT_FAIR, 1, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_FAIR, 2, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_FAIR, 3, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_FAIR, 4, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_FAIR, 5, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_FAIR, 6, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_FAIR, 7, HG_EXTENDED_PARRY_STUN, 0);
set_hitbox_value(AT_FAIR, 1, HG_HITBOX_GROUP, 0);
set_hitbox_value(AT_FAIR, 2, HG_HITBOX_GROUP, 0);
set_hitbox_value(AT_FAIR, 3, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FAIR, 4, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FAIR, 5, HG_HITBOX_GROUP, 0);
set_hitbox_value(AT_FAIR, 6, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FAIR, 7, HG_HITBOX_GROUP, 1);
set_hitbox_value(AT_FAIR, 1, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_FAIR, 2, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_FAIR, 3, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_FAIR, 4, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_FAIR, 5, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_FAIR, 6, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_FAIR, 7, HG_FORCE_FLINCH, 0);
set_hitbox_value(AT_FAIR, 1, HG_SHAPE, 0);
set_hitbox_value(AT_FAIR, 2, HG_SHAPE, 0);
set_hitbox_value(AT_FAIR, 3, HG_SHAPE, 0);
set_hitbox_value(AT_FAIR, 4, HG_SHAPE, 0);
set_hitbox_value(AT_FAIR, 5, HG_SHAPE, 0);
set_hitbox_value(AT_FAIR, 6, HG_SHAPE, 0);
set_hitbox_value(AT_FAIR, 7, HG_SHAPE, 0);
